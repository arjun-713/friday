...
if TYPE_CHECKING:
    from .answering.models import EvidenceContext
...
Response contract:
- Be concise and address the user's current observation or question directly.
- State what to check or do, then state what result to report back.
- Include an inline citation marker for every technical instruction, using exactly this format: [source:chunk_id].
- Use only chunk_id values present in the retrieved evidence.
...
def build_messages(query: str, evidence: Sequence[EvidenceContext]) -> list[dict[str, str]]:
    evidence_text = "\n\n".join(
        "\n".join(
            [
