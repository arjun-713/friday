...
def test_chunk_contract_rejects_missing_resolved_document_metadata() -> None:
    ...
    try:
        DocumentChunk(
            ...
            parser="fixture",
            evidence=[
                Evidence(source_file="manual.pdf", page=4, section="Troubleshooting", content="Restart the router.")
            ],
        )
...
def test_chunk_contract_keeps_multi_page_evidence_consistent() -> None:
    ...
    chunk = DocumentChunk(
        ...
        evidence=[
            Evidence(
                source_file="manual.pdf", page=4, section="Troubleshooting > Wi-Fi", content="Restart the router."
            ),
            Evidence(
                source_file="manual.pdf", page=5, section="Troubleshooting > Wi-Fi", content="Verify the connection."
            ),
        ],
    )
        ...
        pages.append(
            {
                ...
                "text": f"**Router Model AX1 User Manual**\n**WARNING:** Disconnect power.\nBody {page_number}",
                "spans": [],
            }
        )
